...
class TBFeedingEnv(AssistiveEnv):
    ...
    def get_food_rewards(self):
        ...
        for f in self.foods:
            ...
            if distance_to_mouth < 0.03:
                ...
                f.set_base_pos_orient(self.np_random.uniform(
                    1000, 2000, size=3), [0, 0, 0, 1])
                continue
    ...
    def _get_obs(self, agent=None):
        ...
        head_pos_real, head_orient_real = self.robot.convert_to_realworld(
    ...
    def reset(self):
        ...
        for i in range(2):
            for j in range(2):
                for k in range(2):
                    batch_positions.append(np.array(
                        [i*2*food_radius-0.005, j*2*food_radius, k*2*food_radius+0.01]) + spoon_pos)
